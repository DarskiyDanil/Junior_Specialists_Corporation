//
//  ExtensionSearchMyDocumentsWindowVC.swift
//  JSCorporation
//
...
class ExtensionSearchMyDocumentsWindowVC: UIViewController {
    ...
    private let everything: UIButton = {
        let btn = UIButton()
        btn.translatesAutoresizingMaskIntoConstraints = false
        btn.setTitle("Все (.doc, .xml, .pdf)", for: .normal)
        btn.setTitleColor(UIColor.cyanCustom, for: .normal)
        btn.contentHorizontalAlignment = UIControl.ContentHorizontalAlignment.left
        btn.titleLabel?.font = UIFont.customFont15
        btn.layer.borderWidth = 0.5
    ...
    private let statusShow: UIButton = {
        let btn = UIButton()
        btn.translatesAutoresizingMaskIntoConstraints = false
        btn.setTitle("Подписан", for: .normal)
        btn.contentHorizontalAlignment = UIControl.ContentHorizontalAlignment.left
        btn.setTitleColor(UIColor.cyanCustom, for: .normal)
        btn.titleLabel?.font = UIFont.customFont15
        btn.layer.borderWidth = 0.5
    ...
    private func createdByView() {
        ...
        self.view.addSubview(statusShow)
    }
    ...
    private func anchorConstraint() {
        ...
        //status
        self.status.topAnchor.constraint(equalTo: self.spacingPublicationDate.bottomAnchor, constant: 10).isActive = true
        self.status.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: 10).isActive = true
        self.status.trailingAnchor.constraint(equalTo: self.view.trailingAnchor, constant: -10).isActive = true
        self.status.heightAnchor.constraint(equalToConstant: 25).isActive = true
        
        //statusShow
        self.statusShow.topAnchor.constraint(equalTo: self.status.bottomAnchor, constant: 10).isActive = true
        self.statusShow.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: 10).isActive = true
        self.statusShow.trailingAnchor.constraint(equalTo: self.view.trailingAnchor, constant: -10).isActive = true
        self.statusShow.heightAnchor.constraint(equalToConstant: 30).isActive = true
    }
}
